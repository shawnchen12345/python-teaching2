...
| 数据结构 | 符号 | 特性关键词 | 适用范围 (When to use) | 真实案例 |
| :--- | :--- | :--- | :--- | :--- |
| **List 列表** | `[]` | 有序、可变 | 需要**顺序**存储、频繁**修改**的数据 | 购物车商品、排队队列、历史记录 |
| **Dict 字典** | `{}` | Key-Value、极速 | 需要通过**Key查找**、统计数据 | 学生档案、词频统计、API响应数据 |
| **Tuple 元组** | `()` | 有序、**不可变** | **固定**也不希望被改的配置 | 经纬度 `(x, y)`、数据库连接配置 |
| **Set 集合** | `{}` | 无序、**不重复** | **去重**、数学集合运算 (交/并/差) | 共同好友、抽奖名单去重 |
| **String 字符串**| `""` | 文本、不可变 | 文本处理、切片分析 | 用户名、日志、作为字典的Key |
🏁 第15页：课后思考
